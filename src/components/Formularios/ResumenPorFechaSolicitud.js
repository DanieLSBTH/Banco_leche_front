...
const ResumenPorFechaSolicitud = () => {
  ...
  return (
    <Container>
      ...
      <Button color="secondary" onClick={handlePrint} className="mb-3">
        ...
        <tbody>
          {resumen.length > 0 ? (
            resumen.map((servicio, index) => (
              <tr key={index}>
                ...
                <td>{servicio.total_registros_unicos}</td>
          ...
          {resumen.length > 0 && (
            <tr>
              ...
              <td><strong>{totales.totalRegistrosUnicos}</strong></td>
